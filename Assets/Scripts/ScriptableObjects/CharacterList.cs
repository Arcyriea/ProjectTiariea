...
public class Character : ScriptableObject
{
    ...
    public ShieldType ShieldIdentity { get; private set; }
    public float defense { get; private set; } //damage reduction stat
    public float? maximumEnergy { get; private set; } //Used for Shield regeneration if either TECNICAL or HYBRID, can also be used to power up energy weaponries
    public float? maximumMana { get; private set; } //For characters that is capable of arcanic capability, could act as alternate for shield generation if their type is ARCANIC or HYBRID
    public int? maximumAmmo { get; private set; } //For ranged characters which uses physical ammunitions
    public float? maximumGas { get; private set; } //For ranged char that uses liquid to fuel their gas based weaponries
    ...
    public InnateSkill innateSkill { get; private set; } //native character skill
    public ActiveSkill activeSkill { get; private set; } //additional skill, which any char can have

    public CharacterPerk characterPerk { get; private set; }
    
    public PassivePerk passivePerk { get; private set; } //each chars will have an unique passive perk
    ...
    public Character(Sprite sprite, Animator animator, string id, string name, float health, float? shield, float? shootingRange, float meleeRange, float? rangedDamage, float meleeDamage, float swingTime, float? fireRate
        , CharacterPerk characterPerk, PassivePerk passivePerk, InnateSkill innateSkill, ActiveSkill activeSkill)
    {
...
[CreateAssetMenu(menuName = "Skill Asset")]
public abstract class Skill : ScriptableObject
{
    public abstract void function();
}

[CreateAssetMenu(menuName = "Passive Asset")]
